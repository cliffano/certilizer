...
clean:
	rm -rf stage *.egg-info build dist docs/ certilizer/_pycache_/ certilizer/*.pyc tests/_pycache_/ tests/*.pyc .coverage
...
doc: stage
	...
	sphinx-apidoc -o stage/doc/sphinx/ --full -H "certilizer" -A "Cliffano Subagio" certilizer && \
		cd stage/doc/sphinx/ && \
		make html && \
...
lint: stage
	...
	pylint certilizer/*.py tests/*.py tests-integration/*.py
	pylint certilizer/*.py tests/*.py tests-integration/*.py --output-format=pylint_report.CustomJsonReporter > stage/lint/pylint/report.json
	pylint_report stage/lint/pylint/report.json -o docs/lint/pylint/index.html

complexity: stage
	wily build certilizer/
...
coverage:
	COVERAGE_FILE=.coverage.unit coverage run --source=./certilizer -m unittest discover -s tests
	coverage combine
	coverage report
	coverage html

install: package
	pip3 install dist/certilizer-`yq -r .version conf/info.yaml | sed "s/-/_/g"`-py3-none-any.whl

uninstall:
	pip3 uninstall certilizer -y
